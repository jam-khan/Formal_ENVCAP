...
/-
    ...
    Supporting literal init, unit (top), arrow, intersection and record types.
...
inductive Lookup : Typ → Nat → Typ → Prop where
    | lzero :
        ∀ A B,  Lookup (Typ.and A B) 0 B
    | lsucc :
        ∀ A B n C,  Lookup A n C →
                    Lookup (Typ.and A B) (n + 1) C
...
inductive Value : Exp → Prop where
    ...
    | vunit     :
        Value Exp.unit
    | vclosl    :
        ∀ v A e, Value v → Value (Exp.clos v A e)
    | vrcd      :
        ∀ l v, Value v → Value (Exp.erec l v)
    | vmrg      :
        ∀ v1 v2, Value v1 → Value v2 → Value (Exp.mrg v1 v2)

inductive Lin : String → Typ → Prop where
    | lin_rcd   :
        ∀ A l, Lin l (Typ.rcd l A)
    | lin_andl  :
        ∀ A B l,
            Lin l A → Lin l (Typ.and A B)
    | lin_andr  :
        ∀ A B l,
        Lin l B →
        Lin l (Typ.and A B)
...
theorem value_weaken :
    ∀ E v A,
        Definitions.HasType E v A →
        ∀ E', Definitions.Value v →
        Definitions.HasType E' v A :=
    by  intro E v A H E' Hv
        induction H generalizing E'
        case tctx E'                =>  cases Hv
        case tint E n               =>  exact HasType.tint E' n
        case tunit E                =>  exact HasType.tunit E'
        case tapp B e1 e2 A B h1 h2 =>  cases Hv
        case tbox E A B e1 e2       =>  cases Hv
        case tmrg E A B e1 e2       =>
            apply HasType.tmrg
            · cases Hv; apply e1; assumption
            · cases Hv; apply e2; assumption
        case tlam E A B e1 e2           =>  cases Hv
        case tproj E A e B n            =>  cases Hv
        case tclos E' E1' A' e' ev' B'  =>
            cases Hv; apply HasType.tclos; repeat assumption
        case trcd E e A l           =>
            cases Hv; apply HasType.trcd; apply l; assumption
        case trproj E e B l A       =>  cases Hv
